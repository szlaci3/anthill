...
import { Button, Card, Input, Tabs, Drawer, message, Spin, Modal } from 'antd';
import { toArrayIfPossible } from '@/utils/utils';
import VirtualTable from '@/components/VirtualTable';
import classNames from 'classnames';
import { getAllInform, getFnorCount, getFnorItem } from './service';
...
function MessagesUnread() {
  ...
  const res = useRequest((params) => api({ ...params, ...paramsStatus, read: 0}),
    {
  ...
  const resCount = useRequest(getFnorCount,
    {
  ...
  const totalCount = resCount.data ? (+resCount.data.policy || 0) + (+resCount.data.company || 0) : " ";


  return (
    <MessagesTabs>

      <Tabs onChange={onTabChange} type="card" className="search-line-tabs">
        <TabPane tab={<div>所有消息 ({totalCount})</div>} key="所有消息">
          <div className="complement-tabs"/>
          {renderContent()}
        </TabPane>
        <TabPane tab={<div>政策更新 ({resCount.data ? resCount.data.policy : " "})</div>} key="政策更新">
          <div className="complement-tabs"/>
          {renderContent()}
        </TabPane>
        <TabPane tab={<div>企业公告 ({resCount.data ? resCount.data.company : " "})</div>} key="企业公告">
          <div className="complement-tabs"/>
          {renderContent()}
        </TabPane>
      </Tabs>
