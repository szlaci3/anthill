...
export default function myRequest(
  ...
  return (method === "GET" ?
    axios.get(url, newOptions) :
    axios.post(url, newOptions.body, {...newOptions, body: undefined}))
    .then(res => {
      const response = res.data;
      let isHtmlRes = typeof response !== "object";
      if (pipeErrorHandling && (isHtmlRes || response.code == 0 || response.status == 0)) {
        message.error(response.msg || response.info || (isHtmlRes ? "请求错误。" : "请求错误~"));
      }
      return isHtmlRes ? {code: 0, status: 0, htmlRes: 1} : response;
    })
    .catch(e => {
      const status = e.name;
