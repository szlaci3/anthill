...
function DE(props) {
  ...
  let isEmptyDep = true; // Empty has no "open-close" arrow
  if (!props.de.isStaff) {
    if (!props.de.sub[0]) {
      isEmptyDep = true;
    } else {
  ...
  let draggable = props.de.isStaff && !(props.selectedEmployee && props.selectedEmployee.id);

  return <div className={"de " + (props.de.isStaff ? "emp" : "dep") + (props.isOpen ? " open" : "") + (isEmptyDep ? " empty" : "")}>
    <span draggable={draggable} className={"item" + (isDragged ? " dragged-item" : "")} onMouseUp={draggable ? props.handleClickEmployee.bind(null, props.de) : toggleFolder} onMouseDown={props.handleMouseDown.bind(null, props.de)} onDragStart={props.firefoxDragStart} onDragEnd={props.handleDragEnd}>
