...
import { clockImport } from './service';
...
const ScheduleUploadPopup = (props) => {
  const {visible} = props;
  ...
  return (
    <Modal
      ...
      onOk={async () => {
        ...
        let saveRes = await clockImport({file_path: file});
        setIsSaveLoading(false);
        if (saveRes.code == 0) {
          return false;
        }
        message.success(saveRes.msg);
        props.applyImport(saveRes.data.data);
        return true;
      }}
    >
      <Upload
      ...
      <div className="free-line">
        ...
        <a className="linkish" href={extendUrl + "/index/Clock_Info/clockExport?month=" + props.month + "&data=[]"} >点击下载</a>
      </div>
...
export default ScheduleUploadPopup;